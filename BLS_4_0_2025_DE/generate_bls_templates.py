...
Ausgabe: ingredients_details_out/bls_4.0_<name>.json

Verwendung:
    python generate_bls_templates.py [--bls <pfad>] [--out <pfad>] [--dry]

Optionen:
    --bls <pfad>    Pfad zur BLS XLSX Datei (default: BLS_4_0_Daten_2025_DE.xlsx)
    --out <pfad>    Ausgabe-Ordner (default: ../ingredients_details_out/)
...
def row_to_json(row: tuple, col_idx: dict, ingredient_name: str) -> dict:
    ...
    def mv(bls_key):
        """Gibt None zurück wenn Wert 0 ist."""
        val = v(bls_key)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="BLS 4.0 → Template Generator (alle Einträge)"
    )
    parser.add_argument(
    ...
    for idx, row in enumerate(rows, 1):
        if not row[1]:  # Kein Name
            skipped_count += 1
            continue

        ingredient_name = str(row[1]).strip()
        bls_id = str(row[0]).strip() if row[0] else "unknown"

        # Dateiname: bls_4.0_<normalized_name>.json
    ...
    if args.dry:
        print(f"  (DRY RUN – nichts gespeichert)")
